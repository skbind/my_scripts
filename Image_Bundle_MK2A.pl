...
print " ============ updating the view to the LATEST ========== \n";
				$mail_data = '<br><br>Image  doesn\'t exist in archive: <br><br>';

				&_notify_me("$branch_name Bundling notification", $mail_data);
print " just before dying\n";
die;
exit;
...
foreach $line(<$FH>){
	if ($line =~ /^\#/){
		...
		$to_perl=decode_json($json_str);
			...
			my $dir_stats = chdir $img_location;
			...
			if($dir_stats){
				...
				print "link path [$img_location] is not pointing correctly for image:$image\n\n";
				$mail_data = '<br><br> Bundling link $img_location is not pointing to currect path <br><br> chdir failed to cd $img_location <br><br>';
				&_notify_me("$branch_name Bundling notification", $mail_data);
				next;
			}
			if(! -f "$img_location/$img_tar"){
				$mail_data = '<br><br>Image $image doesn\'t exist in archive: $img_location/$img_tar <br><br>';
				&_notify_me("$branch_name Bundling notification", $mail_data);
			...
			$bundling_path .= $source_img.'<br><br>' if ($source_img);
...
&_notify_me("$branch_name Bundling notification", $bundling_path);
	...
	print "========= Preparing for commit as the Images exists in ProcessedImages Array ========\n\n";
		...
		print "========== Performing the commit ============\n\n";
        	...
        	if($? == 0){
               		...
               		&_notify_me("$branch_name Bundling notification", "Commit is successful");
			print "============== Running Mutisite Sync =============== \n\n";
		        ...
		        #print "RunMutiSiteSync($branch_name,$build_partition)";
                	...
                	print "Commit failed \n";
                	&_notify_me("$branch_name Bundling notification", "Commit failed");
                	print "@output\n";
        	}
        } 
	else{
...
sub send_mail{
        ...
        if(!defined($error_email_to)){
                $error_email_to="sabind\@cisco.com";
        }
        else{
...
sub _notify_me{
    ...
    my ($subject,$HTML_data) = @_;
    my $user = 'sabind';
    my $sendto = 'sabind';
    chomp( $user, $sendto, $subject);
...
my $msg = MIME::Lite->new(
    ...
    $msg->attach(
        ...
        Data => qq{
					...
					<p><p>
						This is notification mail from Bundling.
					<p><p><p><p><p>
