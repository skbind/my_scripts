...
if(@processedImages){
	...
	print "PREPARE COMMAD :  cleartool setview -exec \" cd /vob/ios; prepare -f -noip -i $bugid -k 'Bundle porter tar file to Sup2T image' -m LATEST\" $view_name\n"; 
        my @output2=`cleartool setview -exec "cd /vob/ios ; prepare -f -noip -i $bugid -k 'Bundle porter tar file to Sup2T image' -m LATEST" $view_name`;
        if($? == 0){
		print "@output2\n";
