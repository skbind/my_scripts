...
foreach my $data (@r_data) {
    ...
    foreach my  $row (@eachrow) {
    	$row = 'NOT FOUND' unless ( defined $row); 
        	my $value;
        	if ($count >= 5 && $count <= 6){ # start and end time
        		 if($row ne 'NOT FOUND'){
        		 ...
        		 print "$value";
        	}elsif($count == 10){ # Total image list
        		$total_images = $row;
        	}elsif($count == 11){ # Toatal no of list
        		if($row == 0){
