    ...
    foreach my  $row (@eachrow) {
    	...
    	my $value;
  		if($count == 6){ # Total image list
        	...
        	#$value =  "<button onclick=\"javascript:popUp('viewimage.cgi?bug_list=$row')\">buglist</button>" ;
...
my @failed_image;
my $view='/san1/CPY-v151_2_sy_throttle.V151_2_SY6/vob/ios/sys';

foreach my $id (keys %failure_hash){
	@failed_image = @{$failure_hash{$id}};
	my $server = shift @failed_image;
}


my $path_to_program = "/auto/web-cosi/dev/cgi-bin/BUILD_TRACK/test.pl";
my $return_value = system("/router/bin/perl",$path_to_program);
print "<br> ###$return_value :$?:###<br>";

=pod
my %image_map;
#foreach my $img (@failed_image){
	my $cmd = 'ssh build-lnx-011 ; /router/bin/python /router/bin/iosviewinfo --image=c7lc-sp-m.ucode.SSA --view=/san1/CPY-v151_2_sy_throttle.V151_2_SY6/vob/ios/sys'; 
	#my $res = `$cmd`; 
	system ("$cmd");
	if($? == 0){
		print "sucesss : $! <br>";
