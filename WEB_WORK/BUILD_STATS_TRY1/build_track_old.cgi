...
my @build_data;
sub _get_build_data{
	my $run_querry = qq{SELECT ID,SEQ,LABEL_NAME,LOGDIR,TOTAL_IMAGE,BUILT_IMAGE,FAILED_IMAGE FROM BUILD_DETAILS ORDER BY ID };
	my $sth_run = $dbh_sqlite->prepare($run_querry);
	$sth_run->execute();
	while( my @row = $sth_run->fetchrow_array) {
		push @build_data,[@row];
...
sub _table4{
    ...
    print $cgi->Tr($cgi->th({colspan => 1},['ID','SEQ','LABEL_NAME','LOGDIR','TOTAL_IMAGE','BUILT_IMAGE','FAILED_IMAGE']));
